import * as Bcal from "https://cdn.jsdelivr.net/npm/burmese-calendar@1.1.2/+esm";

const calEl = document.getElementById("cal");
const dv = document.getElementById("dv");
const bcal = new Bcal.BurmeseCal();
const dt = new Date();
// ------------------------------------
 ...
 * @type {number[]}
 */
const years = Bcal.numberRange(2000, 2100);
/**
 * @type {string[]}
 */
...
yselect.setAttribute("aria-label", "year");
years.map((i) => {
...
mselect.setAttribute("aria-label", "month");
months.map((i) => {
  const opt = document.createElement("option");
...
lselect.setAttribute("aria-label", "langs");
...
function render() {
  ...
  // blank cells
  const { before, after } = Bcal.blankCells(fwdid, dim);
  ...
  for (let i = 0; i < dim; i++) {
    ...
    if ((wdid === 0 || wdid === 6 || dView[i].isHoliday) && dView[i].isNow) {
      datespan.classList.add("today-h");
    }
    ...
    day.addEventListener("click", (e) => {
      ...
      <div class="dvtext">
          ...
          <span class="dvspan">${d.year.str}-${d.month.long}-${d.day.str}</span>
